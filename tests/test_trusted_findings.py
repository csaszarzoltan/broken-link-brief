...
def attempts(status):
    return [
        ProbeAttempt("HEAD", status, None, 0.01),
        ProbeAttempt("GET", status, None, 0.02),
    ]
...
def test_transient_does_not_create_finding(tmp_path: Path):
    ...
    assert (
        service.observe("p", occ, [ProbeAttempt("HEAD", None, "timeout", 0.1)]) is None
    )
...
def test_lifecycle_versioning_ignore_and_reopen(tmp_path: Path):
    ...
    ignored = store.ignore(
        finding["id"], acknowledged["version"], "expected outage", None
    )
...
def test_detailed_probe_retries_and_preserves_legacy_result():
    ...
    obs = scan_link_detailed(
        "https://example.test/x",
        requester=lambda u, m, t: next(values),
...
def test_detailed_probe_uses_real_local_http_io():
    ...
    try:
        obs = scan_link_detailed(
            f"http://127.0.0.1:{server.server_port}/missing", sleeper=lambda n: None
        )
...
def test_all_verification_outcomes_and_occurrence_reconciliation(tmp_path: Path):
    ...
    unknown = service.verify(
        finding["id"],
        finding["version"],
        [ProbeAttempt("HEAD", None, "timeout", 0.1)],
...
def test_archived_project_is_read_only(tmp_path: Path):
    ...
    service = FindingService(store)
    occurrence = extract_occurrences("https://site.test/", '<a href="/x">x</a>')[0]
...
def test_evidence_errors_redact_secret_values(tmp_path: Path):
    ...
    finding = FindingService(store).observe(
        "p",
        occurrence,
        [ProbeAttempt("HEAD", None, "token=supersecretvalue", 0.1)],
    )
...
def test_new_outbound_paths_validate_urls_before_fetching():
    ...
    project_block = source[
        source.index("for occurrence in extract_occurrences") : source.index(
            "# Record scan and trigger"
        )
    ]
    verify_block = source[
        source.index("def _verify_finding") : source.index(
