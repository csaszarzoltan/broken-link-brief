...
def test_us_002_queued_cancel_calls_no_scanner(tmp_path):
    p = ProjectStore(tmp_path / "x.db").create("P", ["https://example.com/"])
    j = ScanJobStore(tmp_path / "x.db")
    job = j.create(p.id, p.name, list(p.targets))
    out = j.cancel(job["id"], job["version"])
...
def test_us_002_terminal_cancel_conflicts(tmp_path):
    ...
    j = ScanJobStore(tmp_path / "x.db")
    job = j.create(p.id, p.name, list(p.targets))
    job = j.cancel(job["id"], job["version"])
    with pytest.raises(JobConflict):
        j.cancel(job["id"], job["version"])
